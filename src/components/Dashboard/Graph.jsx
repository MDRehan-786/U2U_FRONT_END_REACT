...
import { FaShoppingCart,FaChartLine, FaLevelUpAlt, FaArrowUp,FaMoneyBillWave  } from "react-icons/fa";
...
function Graph() {
  ...
  const series = [parseFloat(dashboardData?.daily_profit)+parseFloat(dashboardData?.level_profit), dashboardData?.pending_roi];
        ...
        <div className="py-3 flex gap-3 sm:flex-row flex-col">
          <div className="text-sm flex flex-col gap-5">
            <div className="flex items-center gap-2">
              <IoIosSettings className="text-[#38C66C]" /> Total Delegator Amount - $
              {dashboardData?.total_investment.toFixed(4)}
            </div>
            <div className="flex items-center gap-2">
              <GrStakeholder className="text-orange-500" /> Stake Amount- $
              {dashboardData?.stake.toFixed(4)}
            </div>
            <div className="flex items-center gap-2">
              <GiChessQueen className="text-sky-600" /> Restake Amount- $
              {dashboardData?.restake.toFixed(4)}
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              Earned Daily Reward - $ {dashboardData?.daily_profit.toFixed(4)}
            </div>
             <div className="flex items-center gap-2">
             <FaChartLine className="text-[#4E7AD9]" /> 
              Earned Level Reward - $ {dashboardData?.level_profit.toFixed(4)}
            </div>
             <div className="flex items-center gap-2">
           <FaMoneyBillWave className="text-[#4E7AD9]" /> 
              Earned Total Reward - $ {(parseFloat(dashboardData?.daily_profit)+parseFloat(dashboardData?.level_profit)).toFixed(4)}
            </div>
            <div className="flex items-center gap-2">
